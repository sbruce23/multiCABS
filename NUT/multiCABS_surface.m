function[spectra, coh] = multiCABS_surface(zt, spect_matrices, fit, params) 


dim1 = size(zt); dim2 = size(zt{1});
nobs = dim1(1); dimen=dim2(2);
    ...
    for p=1:params.nloop
        if(p>params.nwarmup)
            ...
            for j=1:fit(1).nexp_curr(p)
                if(j==1)
                    s_11(:,1:xi_curr(j))=s_11(:,1:xi_curr(j))+repmat(spec_hat_curr_11(:,j),1,xi_curr(j))/(params.nloop-params.nwarmup);
                    s_22(:,1:xi_curr(j))=s_22(:,1:xi_curr(j))+repmat(spec_hat_curr_22(:,j),1,xi_curr(j))/(params.nloop-params.nwarmup);
                    s_33(:,1:xi_curr(j))=s_33(:,1:xi_curr(j))+repmat(spec_hat_curr_33(:,j),1,xi_curr(j))/(params.nloop-params.nwarmup);
                else

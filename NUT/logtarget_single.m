function [log_target] = logtarget_single(yobs, Beta)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global dimen nBeta
Beta_tmp = reshape(Beta,nBeta,dimen^2);
Beta_1 = Beta_tmp(:,1:(dimen + dimen*(dimen-1)/2));
Beta_2 = Beta_tmp(:,(dimen + dimen*(dimen-1)/2 + 1): end);
...
if dimen==2
    if (mod(n,2)==1) %odd n
           log_target = -sum(log(delta_sq(1,2:end))' + log(delta_sq(2,2:end))' + ...
           ...
           log_target = -sum(log(delta_sq(1,2:nfreq))' + log(delta_sq(2,2:nfreq))' + ...
...
elseif dimen==3
    if (mod(n,2)==1) %odd n
        log_target = -sum(log(delta_sq(1,2:end))' + log(delta_sq(2,2:end))' + log(delta_sq(3,2:end))' + ...
                        conj(y(2:end,1)).*y(2:end,1).*exp(-xx_r(2:end,:)*Beta_1(:,1)) + ...
        ...
        log_target = -sum(log(delta_sq(1,2:nfreq))' + log(delta_sq(2,2:nfreq))' + log(delta_sq(3,2:nfreq))' + ...
                        conj(y(2:nfreq,1)).*y(2:nfreq,1).*exp(-xx_r(2:nfreq,:)*Beta_1(:,1)) + ...

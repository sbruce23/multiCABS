function[A,nseg_new,xi_prop,tau_prop,Beta_prop,seg_temp]=...
        within(ts,nexp_temp,tau_temp,xi_curr_temp,nseg_curr_temp,Beta_curr_temp)
global nobs dimen nBeta M prob_mm1 umin
...
if nexp_temp>1
    ...
    for j=seg_temp:seg_temp+1
        ...
        [Beta_out, m_out, m, yobs_tmp] = Hamilt(j, ts, tau_prop(:,j), Beta_prop(:,:,j), xi_prop);                            
        if j==seg_temp
        ...
        m_curr = m_curr - 0.5*m'*((1/M)*eye(length(m)))*m;                      
        m_prop = m_prop - 0.5*m_out'*((1/M)*eye(length(m_out)))*m_out; 
    end   
    ...
    log_target_prop = loglike_prop + log_prior_cut_prop +  m_prop;
    log_target_curr = loglike_curr + log_prior_cut_curr +  m_curr;
    ...
    [Beta_out, m_out, m]=Hamilt(1, ts, tau_temp, Beta_curr_temp, xi_curr_temp);
    ...
    log_target_prop = loglike_prop/(nobs/2) + m_prop;
    log_target_curr = loglike_curr/(nobs/2) + m_curr;
    log_proposal_curr =  0;
    log_proposal_prop =  0;
end

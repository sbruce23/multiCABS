function [f,gr,h] = Beta_derive_single(x, yobs, tau_temp, sigmasqalpha, nbasis)

global dimen
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Function used for optimization process for coefficients selected to
% be changed
%
%   Input:
%       1) x - initial values for coeeficient of basis functions need to be
%       optimized 
%       2) yobs - time series data within the segment
%       3) tau_temp - smoothing parameters  
%       4) Beta_temp - current coefficients
%       5) sigmasqalpha - smoothing parameters for the constant in real
%           omponents
%       7) nbasis - number of basis function used
%   Main Outputs:
...
dim = size(yobs); n = dim(1);
nfreq = floor(n/2); tt = (0:nfreq)/(2*nfreq);
yy = fft(yobs)/sqrt(n); y = yy(1:(nfreq+1),:); nf = length(y);
...
if dimen==2  %Bivariate Time Series
   ...
   f = f - (0.5.*(Beta_1(1,1)* Beta_1(1,1)')/sigmasqalpha + 0.5.*(Beta_1(2:nBeta,1)'*Beta_1(2:nBeta,1))/tau_temp(1)) -...
